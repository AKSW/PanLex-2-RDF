...
\begin{abstract}
At present, there are approximately 6,900 actively spoken languages in the world.
However, some experts claim, that the process of globalization may eventually lead
to a world loosing this linguistic diversity.
The vision of the PanLex project is to save these languages, especially low density ones,
by allowing them to be a part of the Information Age by making them applicable via translations.
For this reason, PanLex gathers and integrates information from several
linguistict sources, such as monolingual dictionaries, bilingual dictionaries, multilingual dictionaries,
...
% \end{itemize}
At present, there are about 7,000 languages in the world with approximately 6,900 of them being actively spoken\footnote{see \url{http://www.sil.org/iso639-3/download.asp} for a list of registered languages}.
Nonetheless, some experts claim, that the process of globalization may lead to a monolingual world loosing this linguistic diversity~\cite{endang_lang}.
Since there are a lot of low density languages that were not taken over from generation to generation anymore, we could irreversibly loose parts of human knowledge.
Some predictions state that this decline may lead to a loss of half to 90\% of human's languages~\cite{endang_lang}.

The vision of the PanLex project is to save all these thousands of languages, especially those low-density ones that are threatened by extinction.
PanLex tries to provide means for panlingual translations from any language of the world into any other.
But it is not feasible to just store all the translations of one word into any other language since this would result in trillions of translations.
So the PanLex project additionally uses artificial intelligence techniques to infer translations.
As a consequence, not just mainstream languages like English, but even these low-density languages may be used for a global communication.
% Since it is not feasible to just store all the translations of one word into any other language, which would result in trillions of translations, a system is needed that allows translations by inference using artificial intelligence.

Currently there is a growing community trying to combine linguistic knowledge with Semantic Web technologies and so build a web of linguistic data, also known as the \emph{Linguistic Linked Open Data (LLOD) cloud}.
Originally the PanLex project provides different web services\footnote{\url{http://panlex.org/try/}} and APIs\footnote{\url{http://panlex.org/tech/doc/api/}} to access its data.
Since the project shares the idea of open access we tried to make PanLex a part of the LLOD cloud.

In \autoref{sec:triplificate} we will introduce the PanLex dataset, present our PanLex RDF vocabulary and explain how we transformed the one into the other.
The following section is about the actual dataset publishing and how we linked to other datasets of the LLOD cloud.
...
Subsequently, we will explain the steps taken to publish the data as RDF.
The vocabulary introduced for the PanLex dataset will be described and afterwards we will show how the original data was converted to RDF and published via a SPARQL endpoint.
The interlinking of the created RDF data with other datasets will be discussed in the following section.
...
The database contains about 20 million meanings and 19 million expressions extracted from about 4.000 sources.

Assertions are factual claims attributed to a certain source and editor.
They do not represent absolute truths and as such it is valid that different editors make contradictory assertions on different sources.
...
bilingual dictionaries, multilingual dictionaries, glossaries, standards, thesauri and so on.
...
\begin{itemize}
  ...
  They were extracted from the above mentioned mono- or bilingual dictionaries or from other sources.
  ...
  \item \emph {Language Varieties} within a language are further distinguished by custom names.
  For example, ``eng'' is the ISO 639-3 code for the English language.
  Panlex defines ``simple english'' and ``british english'' as varieties of it.
  These can be given in the native writing system.
  ...
  Additionally these variety names do not have to be registered like the IETF BCP 47 language subtags\footnote{\url{http://www.iana.org/assignments/language-subtag-registry}}.
  \item \emph{Meanings} of expressions are stated by users based on their
  interpretation of an information source.
  Information sources can be dictionaries, thesauri but also personal knowledge. 
  The pair comprised of the user and the information source is referred to as a meaning's \emph{approver}.
  Users may add informal definitions to meanings in any language variety.
  \item \emph{Definitions} are optional descriptions of a meaning.
  They are given in a certain language variety.
  A description of the verb \emph{browse} for example could be \emph{``move or surf through various files on a computer, the Internet, etc.''}.
  \item \emph{Denotations} relate expressions to possible \emph{meanings}.
  So homonyms are represented by one expression being connected to different meanings.
  Denotations may be enriched with parts of speech tags adopted from the
  ...
  Furthermore, it is possible to generically tag denotations with meta data
  ...
  \item \emph{Users} are related to their defined language varieties and edited approvers.
  \item \emph{Licenses} are also considered by the PanLex project.
  At present there are ten different license categories an approver can get into.
  They are \emph{public domain}, \emph{Creative Commons}, \emph{request} (meaning that one has to ask the author of the source at hand), \emph{GNU General Public License}, \emph{GNU Lesser General Public License}, \emph{GNU Free Documentation License}, \emph{MIT License}, \emph{copyright} (stating that there is a certain copyright holder), \emph{other} and \emph{unknown}.
\end{itemize}

An overview of the number of instances per entity in the PanLex database is given in~\autoref{fig:plx-entity-counts}.
\begin{table}
\centering
\begin{scriptsize}
\begin{tabular}{lr}
...
There are two noteworthy limitations of the database schema:
Approver combines a user with an information source, however the \emph{information source} is not modelled as a distinct entity.
Users are only associated with language variety and approver entities. However,
it is not possible to track which denotations were added by users.
\todo{What are the consequences? }
...
Since the relational schema is a good starting point most of the relations were taken over introducing a PanLex RDF vocabulary.
Therefore the namespace \texttt{<http://panlex.org/plx/>} is used, abbreviated by the \texttt{plx} prefix.

The structure of \emph{approvers} is nearly the same as in the database schema.
All approvers share the \texttt{plx:Approver} class.
The characteristics of an approver are described using mainly triples with literal objects.
These are for example \texttt{plx:registrationDate} pointing to an \texttt{xsd:date} literal, \texttt{dc:title} to assign the title of a source, \texttt{dc:creator} to give an \texttt{xsd:string} containing the author's name etc.
...
\emph{ISO 639-1} and \emph{ISO 639-3 codes} are of the classes \texttt{plx:Iso639-1Code} and \texttt{plx:Iso639-3Code} respectively.
...
The RDF analogon of the PanLex \emph{meaning} is the \texttt{plx:Meaning}.
Entities of this class may have an identifier assigned with the \texttt{plx:identifier} property pointing to an \texttt{xsd:string} literal.
This meaning identifier represents ids given to the considered meaning in other data sources and may be used to build links to the meaning as represented on these sites.
Meanings may also have \emph{definitions}, entities of the \texttt{plx:Definition} class, giving a textual representation (\texttt{rdfs:label}) with more than just a lexeme in a certain language variety (\texttt{plx:languageVariety}).

Following the semantics of the PanLex database, meanings and expressions are linked via \emph{denotations}.
...
Since the PanLex project compiled its database by extracting data from different sources, the licenses of these sources were considered, too.
...
In the RDF dataset these have an additional \texttt{rdfs:label} property giving the names or descriptions of the licenses or license categories.
\begin{table}
\begin{tabular}{ll}
Class & Properties \\
\toprule
...
\bottomrule
\end{tabular}
\caption{Classes and properties used in the PanLex RDF vocabulary}
...
\end{figure}
There, any \texttt{rdf:type} triples as well as facts assigning ISO 639-1 and ISO 639-3 labels are omitted for clarity.

\subsection{Transformation workflow}
\label{sec:conversion}
\todo{how often in PanLex updated?}
The size of the PanLex database (including indexes) is approximately 18 GB,
which is big enough to make recurrent RDF conversion cumbersome:
A full RDF conversion comes bloats in the dataset size and data duplication.   
Using conventional hardware, it takes quite long
to convert all the data, which usually makes
testing and debugging and fixing issues in the modelling or data conversion of the
data to become very time consuming.
As the PanLex data already resides in a relation database, the use of a virtual
RDB 2 RDF mapping solution is a natural choice. The \emph{Sparqlify Mapping Language} thereby
...
adding support for now ones do no require an ETL process, and can therefore be done with little efforts.
...
For each language version we normalized the labels in the same way as being done in PanLex, i.e.
Unicode NFKD\footnote{\url{http://unicode.org/reports/tr15/}} normalization and
removal of punctuation characters are applied.
The DBpedia resources are then mapped to the PanLex expression that are equal to the resource's normalized label in the respective language.
...
the Linked Data\footnote{\url{http://panlex.org/plx/TODO}} and the SPARQL
...
\todo{Where to host the linksets?}
...
ones having low numbers in their speakers.
As such, the extraction of information from linguistic sources, as well as
...
which offers a platform for sharing word nets and defines several goals.
These include setting forth standards for uniformly representing wordnets
of different languages and establishing an universal index of meaning.
...
Thereby, several (quasi-)standard ontologies have been developed for covering different aspects of linguistic resources.
...
the actualy RDF transformation.
Furthermore, we interlinked the languages in PanLex with lexvo,
and created a total of \todo {number} derefenceable
links to DBpedia for expressions in 16 languages.
...
as distinct entities.\todo{We should still map it like that with Sparqlify - resuse ap for the source}
Also, while it is possible to determine
a meaning's source by its approver, this is not possible for the denotations.
Retaining this information seems beneficial for translation approches,
as this enables for example the exclusion of a poor quality or for a certain task
