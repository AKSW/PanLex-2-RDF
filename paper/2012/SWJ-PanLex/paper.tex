...
Theorists of biolinguistic diversity~\cite{nettle} argue that the loss of
...
and revitalization are essential to the preservation of biological diversity.
...
\begin{itemize}
  ...
  \item \emph{Denotations} relate expressions to \emph{meanings}.
...
Users are only associated with language variety and approver entities. However, whenever a given approver has more than one user with editorial privileges, it is not possible to track which denotations were added by which of the approver's authorized users.
...
The RDF analog of the PanLex \emph{meaning} is the \texttt{plx:Meaning}.
...
A full RDF conversion bloats the dataset and results in data duplication.   
